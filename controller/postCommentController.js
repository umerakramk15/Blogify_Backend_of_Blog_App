...
export const fetchAllSinglePostComment = async (req, res) => {
  try {
    const { postId } = req.params;
    const singlePostAllComments = await postCommentModel.find({
      postId: postId,
    });
    if (!singlePostAllComments) {
    ...
    return res.status(200).send({
      success: true,
      message: "Successfuly Fetch All post ",
      singlePostAllComments,
    });
  } catch (error) {
    console.log(error);
    res.status(500).send({
      success: false,
      message: "Something error in detch all single post comment",
    });
  }
};
